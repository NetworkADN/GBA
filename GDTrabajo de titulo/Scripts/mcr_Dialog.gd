...
@onready var dialog_label = $Dialogo/Dialogo_mcr  # Nodo para el texto del diálogo
@onready var name_label = $nombres/caja_nombre  # Nodo para el texto del nombre
@onready var arrow = $"../ParallaxBackground/ParallaxLayer/Casino(microondas)/Button"

func _ready():
	if  DialogsFlags.c2 == false and DialogsFlags.cj == true: 
		# Inicializa el diálogo
		if DialogsFlags.c1 == true and DialogsFlags.c3 == true and DialogsFlags.c4 == true and DialogsFlags.ord == true:
			dialog = [
				think,"¿Eh? Hay algo en la mesa del microondas...",
				think,"¡Es un numero pintado en plumon negro!",
				think,"1... Supongo que es uno de los numeros para el candado",
				obt, "HAS ENCONTRADO LOS CUATRO NUMEROS Y TIENES EL ORDEN CORRECTO, VUELVE AL CASILLERO PARA INTRODUCIR LA CLAVE"
			]
		elif DialogsFlags.c1 == true and DialogsFlags.c3 == true and DialogsFlags.c4 == true and DialogsFlags.ord == false:
			dialog = [
				think,"¿Eh? Hay algo en la mesa del microondas...",
				think,"¡Es un numero pintado en plumon negro!",
				think,"1... Supongo que es uno de los numeros para el candado",
