...
func snap_to_target() -> void:
	...
	position.x = x
	position.y = y
	DialogsFlags.trap = true
	snapped = true  # Bloquea el movimiento

func _on_area_2d_area_entered(area):
	if area.is_in_group("trapezoide"):  # Si el collider es el correspondiente
		print("Detectado area2d")
		target_position = position  # Guarda la posición del collider


func _on_area_2d_area_exited(area):
	if area.is_in_group("trapezoide") and not snapped:  # Si no está colocada, borra el target
		print("salio area2d")
